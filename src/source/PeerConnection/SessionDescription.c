...
STATUS setTransceiverPayloadTypes(PHashTable codecTable, PHashTable rtxTable, PDoubleList pTransceivers) {
    ...
    while (pCurNode != NULL) {
        ...
        if (pKvsRtpTransceiver != NULL &&
            ...
            if (pKvsRtpTransceiver->sender.track.codec ==  RTC_CODEC_H264_PROFILE_42E01F_LEVEL_ASYMMETRY_ALLOWED_PACKETIZATION_MODE ||
                pKvsRtpTransceiver->sender.track.codec == RTC_CODEC_VP8)
            {
